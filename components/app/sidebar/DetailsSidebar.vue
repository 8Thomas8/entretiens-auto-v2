...
<template>
  <aside class="w-full h-full md:border-l md:border-gray-200 bg-white p-8 block relative">
    <div class="fixed top-2">
      <button
          @click="closeSidebar()"
          class="-ml-6 flex items-center rounded-md bg-white p-1 text-indigo-600 hover:text-indigo-500 focus:outline-none">
        <svg
    ...
    <div class="flex flex-col w-full h-full space-y-6 pb-16">
      <div>
        ...
        <div class="mt-4">
          <h2 class="text-lg font-medium text-gray-900">{{ selectedVehicle.name }}</h2>
          <p class="text-sm font-medium text-gray-500">{{ selectedVehicle.builder }}</p>
        </div>
        <div class="mt-4">
          <AtomicBtnPrimary
              :iconSvg="iconBtn"
      ...
      <div class="grow">
        ...
        <dl class="mt-2 divide-y divide-gray-200 border-t border-b border-gray-200">
          <div class="flex justify-between py-3 text-sm font-medium">
            ...
            <dd class="whitespace-nowrap text-gray-900">{{ selectedVehicleKmFormatted }}</dd>
          </div>

          <div class="flex justify-between py-3 text-sm font-medium">
            <dt class="text-gray-500">Energie</dt>
            <dd class="whitespace-nowrap text-gray-900">{{ Energy[selectedVehicle.energy] }}</dd>
          </div>

          <div class="flex justify-between py-3 text-sm font-medium">
            <dt class="text-gray-500">Date de création</dt>
            <dd class="whitespace-nowrap text-gray-900">{{ selectedVehicleCreatedDateFormatted }}</dd>
          </div>

          <div class="flex justify-between py-3 text-sm font-medium">
            <dt class="text-gray-500">Date de mise à jour</dt>
            <dd class="whitespace-nowrap text-gray-900">{{ selectedVehicleUpdatedDateFormatted }}</dd>
          </div>
        </dl>
      </div>
